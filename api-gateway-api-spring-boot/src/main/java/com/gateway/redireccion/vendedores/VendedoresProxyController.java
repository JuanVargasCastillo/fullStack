...
public class VendedoresProxyController {
    ...
    public ResponseEntity<?> proxyVendedores(HttpServletRequest request,
                                             @RequestBody(required = false) String body,
                                             @RequestHeader HttpHeaders headers) {

        String originalPath = request.getRequestURI().replace("/api/proxy/vendedores", "");
        String targetUrl = "http://localhost:8089/api/vendedores" + originalPath; // Cambia el puerto si es necesario
        HttpMethod method = HttpMethod.valueOf(request.getMethod());

        // Validar DELETE solo si es admin
        if (method == HttpMethod.DELETE) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede eliminar vendedores\"}");
            }
        }

        // Validar PUT solo si es admin
        if (method == HttpMethod.PUT) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede modificar vendedores\"}");
            }
        }
